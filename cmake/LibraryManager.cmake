...
Finds a LAPACK library and links it as a requirement for a build target.
...
function(LibraryManager_LAPACK target)
    message(DEBUG "LibraryManager_FindLAPACK for target ${target}")
    cmake_parse_arguments("ARG" "PRIVATE;INTERFACE;PUBLIC" "" "" ${ARGN})
    set(linkType "PUBLIC")
    if (ARG_PUBLIC)
        set(linkType "PUBLIC")
    ...
    #    print_variable(linkType)
    #    message("LibraryManager_FindLAPACK before setting target_link_libraries linkType=${linkType}")
    #    print_target_properties(${target})
    target_link_libraries(${target} ${linkType} LAPACK::LAPACK)
    message(DEBUG "LibraryManager_FindLAPACK after setting target_link_libraries")
    #    print_target_properties(${target})
endfunction()
...
macro(__LibraryManager_findBLASorLAPACK name)
    message(DEBUG "!! Enter __LibraryManager_findBLASorLAPACK ${name}")
    ...
    if (NOT TARGET ${name}::${name})
        ...
        if ("${vendors}" STREQUAL ";")
            set(vendors Intel10_64lp Apple PKG_CONFIG)
        endif ()
        set(MKL OFF)
        set(MKL_TYPE)
        set(BLA_VENDOR_FOUND)
        # facilitate discovery of MKL
        if (NOT DEFINED ENV{MKLROOT})
            foreach (d /opt/intel/mkl /opt/intel/oneapi/mkl/latest ~/intel/mkl ~/intel/oneapi/mkl/latest)
                if (EXISTS ${d})
                    set(ENV{MKLROOT} ${d})
                endif ()
            endforeach ()
        endif ()
        message(DEBUG "ENV{MKLROOT}=$ENV{MKLROOT}")

        foreach (BLA_VENDOR ${vendors} "")
            unset(${name}_FOUND)
            unset(${name}95_FOUND)
            unset(${name}_LIBRARIES)
            unset(${name}95_LIBRARIES)
            message(DEBUG "try BLA_VENDOR ${BLA_VENDOR} for ${name}")
            unset(BLA_PREFER_PKGCONFIG)
            if (BLA_VENDOR STREQUAL "PKG_CONFIG")
                set(BLA_PREFER_PKGCONFIG ON)
                find_package(PkgConfig)
                string(TOLOWER "${name}" names)
                pkg_check_modules(${name} ${names})
                #                print_variable(${name}_FOUND)
                #                print_variable(${name}_LIBRARIES)
                #                print_variable(${name}_LIBRARY_DIRS)
                #                print_variable(${name}_CFLAGS)
                #                list(APPEND CMAKE_LIBRARY_PATH "/opt/homebrew/opt/lapack/lib")
                #                list(APPEND CMAKE_LIBRARY_PATH "/usr/local/opt/lapack/lib")
                #                list(APPEND CMAKE_LIBRARY_PATH "/opt/homebrew/opt/openblas/lib")
                #                list(APPEND CMAKE_LIBRARY_PATH "/usr/local/opt/openblas/lib")
            elseif (BLA_VENDOR STREQUAL "")
                find_package(${name} REQUIRED)
            else ()
                find_package(${name})
            endif ()
            if (${name}_FOUND)
                message(DEBUG "Found ${name} with BLA_VENDOR=${BLA_VENDOR} ${name}_FOUND=${${name}_FOUND} ${name}_LIBRARIES=${${name}_LIBRARIES}")
                set(BLA_VENDOR_FOUND "${BLA_VENDOR}")
                if ("${${name}_LIBRARIES}" MATCHES "Accelerate")
                    set(BLA_VENDOR_FOUND "Apple")
                endif ()
                if ("${${name}_LIBRARIES}" MATCHES "openblas")
                    set(BLA_VENDOR_FOUND "OpenBLAS")
                endif ()
                message(DEBUG "BLA_VENDOR_FOUND initial ${BLA_VENDOR_FOUND}")
                if (NOT TARGET ${name}::${name})
                ...
                if (${name}_C_FLAGS)
                    target_compile_options(${name}::${name} INTERFACE "${${name}_C_FLAGS}")
                endif ()
                if (${name}_LIBRARY_DIRS)
                    target_link_directories(${name}::${name} INTERFACE "${${name}_LIBRARY_DIRS}")
                endif ()
                if (${name}_LIBRARIES)
                    target_link_libraries(${name}::${name} INTERFACE "${${name}_LIBRARIES}")
                endif ()
                if (${name}_LINKER_FLAGS)
                    target_link_options(${name}::${name} INTERFACE ${${name}_LINKER_FLAGS} ${${name}_LDFLAGS})
                endif ()
                #                print_target_properties(${name}::${name})
                message(DEBUG "${name}_LIBRARIES ${${name}_LIBRARIES}")
                string(REGEX MATCH "mkl" _MKLFOUND "${${name}_LIBRARIES}")
                message(DEBUG "_MKLFOUND ${_MKLFOUND}")
                if (_MKLFOUND)
                    set(MKL ON)
                    if (APPLE)
                    ...
                    if (DEFINED ENV{MKLROOT})
                        ...
                        find_path(MKL_INCLUDE_DIR "mkl.h" REQUIRED PATHS "/usr/include/mkl")
                        target_include_directories(${name}::${name} INTERFACE "${MKL_INCLUDE_DIR}")
                    endif ()
                    target_compile_definitions(${name}::${name} INTERFACE USE_MKL)
                    target_compile_definitions(${name}::${name} INTERFACE EIGEN_USE_MKL_ALL)
                    set(_THREAD OFF)
                    foreach (_LIB ${${name}_LIBRARIES})
                        message(DEBUG "_LIB=${_LIB}")
                        if ("${_LIB}" MATCHES "_ilp64[.]")
                            set(BLA_VENDOR_FOUND "Intel10_64ilp")
                        endif ()
                        if ("${_LIB}" MATCHES "_lp64[.]")
                            set(BLA_VENDOR_FOUND "Intel10_64lp")
                        endif ()
                        if ("${_LIB}" MATCHES "intel_thread")
                            set(_THREAD ON)
                        endif ()
                    endforeach ()
                    if (NOT _THREAD)
                        set(BLA_VENDOR_FOUND ${BLA_VENDOR_FOUND}_seq)
                    endif ()
                    ...
                    message(DEBUG "Not Intel; ${name}_LIBRARIES=${${name}_LIBRARIES}")
                    target_compile_definitions(${name}::${name} INTERFACE "-DNO_USE_MKL")
                endif ()
                ...
                if ("${name}" STREQUAL "LAPACK")
                    ...
                    if ("${BLA_VENDOR_FOUND}" STREQUAL "Apple" OR "${BLA_VENDOR_FOUND}" STREQUAL "OpenBLAS" OR LAPACK_LIBRARIES MATCHES "liblapack.|lapack") # special subversion to use generic lapacke together with Accelerate or generic
                        if (APPLE AND "$ENV{HOMEBREW_PREFIX}" STREQUAL "")
                            execute_process(COMMAND brew --prefix OUTPUT_VARIABLE _homebrew_prefix)
                            string(STRIP "${_homebrew_prefix}" _homebrew_prefix)
                            set(ENV{HOMEBREW_PREFIX} "${_homebrew_prefix}")
                        endif ()
                        if ("$ENV{HOMEBREW_PREFIX}" STREQUAL "")
                            set(ENV{HOMEBREW_PREFIX} "/usr/local")
                        endif ()
                        message(DEBUG "HOMEBREW_PREFIX=$ENV{HOMEBREW_PREFIX}")
                        find_library(_private_lapacke_lib "lapacke" PATHS "$ENV{HOMEBREW_PREFIX}/opt/lapack/lib" "opt/local/lib" "/usr/lib" "/usr/lib/x86_64-linux-gnu" "/usr/lib/aarch64-linux-gnu")
                        if (NOT _private_lapacke_lib)
                            message(FATAL_ERROR "Cannot find lapacke library in system libraries, $ENV{HOMEBREW_PREFIX}/opt/lapack/lib or /opt/local/lib. Install lapack package from homebrew or macports.")
                        endif ()
                        find_file(_private_lapacke_h "lapacke.h" PATHS "$ENV{HOMEBREW_PREFIX}/opt/lapack/include" "/opt/local/include/lapack" "/usr/include")
                        if (NOT _private_lapacke_h)
                            message(FATAL_ERROR "Cannot find lapacke.h in $ENV{HOMEBREW_PREFIX}/opt/lapack/include or /opt/local/include/lapack. Install lapack package from homebrew or macports.")
                        endif ()
                        #                        print_variable(_private_lapacke_h)
                        string(REPLACE "/lapacke.h" "" _private_lapacke_include "${_private_lapacke_h}")
                        #                        print_variable(_private_lapacke_include)
                        target_include_directories(${name}::${name} INTERFACE ${_private_lapacke_include})
                        target_link_libraries(${name}::${name} INTERFACE ${_private_lapacke_lib})
                    endif ()
                endif ()
                if ("${name}" STREQUAL "BLAS")
                    target_compile_definitions(${name}::${name} INTERFACE -DEIGEN_USE_BLAS)
                endif ()
                break()
            endif ()
        endforeach ()
    else ()
        message(DEBUG "Target ${name}::${name} already exists")
    endif ()
endmacro()
...
function(LibraryManager_Install target)
    ...
    get_target_property(type ${target} TYPE)
    message(DEBUG "target_include_directories(${target} INTERFACE $<INSTALL_INTERFACE:${ARG_INCLUDE_DIR}>)")
    if (type STREQUAL "INTERFACE_LIBRARY")
    ...
    if (ARG_PKG_CONFIG)
        ...
        string(REGEX REPLACE "\\$<HOST_LINK:SHELL:(.*)> " "\\1 " content "${content}")
        #message("content=${content}")
